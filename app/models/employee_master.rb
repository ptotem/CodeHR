class EmployeeMaster
  ...
  def self.import(file)
    ...
    (2..spreadsheet.last_row).each do |i|
      @employee_master = EmployeeMaster.create!(employee_code:spreadsheet.row(i)[1].to_s,
                                                employee_name:spreadsheet.row(i)[2].to_s,
                                                employee_middle_name_name:spreadsheet.row(i)[3].to_s,
                                                status:spreadsheet.row(i)[4].to_s,
                                                gender:if spreadsheet.row(i)[5].to_s == "Male" then "M" elsif spreadsheet.row(i)[35].to_s == "Female" then "F" else "O" end,
                                                marital_status:spreadsheet.row(i)[6].to_s,
                                                date_of_birth:if spreadsheet.row(i)[7].nil? then "" else spreadsheet.row(i)[7].to_date end,
                                                date_of_joining:if spreadsheet.row(i)[8].nil? then "" else spreadsheet.row(i)[8].to_date end,
                                                designation_joined_at:spreadsheet.row(i)[9].to_s,
                                                official_email:spreadsheet.row(i)[10].to_s,
                                                address_for_communication:spreadsheet.row(i)[11].to_s,
                                                parent_ids:[EmployeeMaster.where(:employee_code => spreadsheet.row(i)[12].to_s).first.id],
                                                role_ids:[Role.where(:name => spreadsheet.row(i)[13].to_s)],
                                                group_master_ids:spreadsheet.row(i)[14].to_s.split(',').map{|i| GroupMaster.where(:group_name => i).first.id})
                                                # parent_ids:
